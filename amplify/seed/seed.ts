  ...
  const response = await dataClient.models.Patient.create(
    {
      ...
      emergencyContact: `Contact ${['Smith', 'Jones', 'Williams'][Math.floor(Math.random() * 3)]} +1-555-${String(Math.floor(Math.random() * 10000000)).padStart(7, '0')}`,
      ...
      allergies: `Known allergies: ${['None', 'Peanuts', 'Pollen', 'Dust Mites', 'Shellfish'][Math.floor(Math.random() * 5)]}. Notes: ${Math.random().toString(36).substring(2, 10)}.`,
